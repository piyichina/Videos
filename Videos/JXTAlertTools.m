...
//类别，扩充属性
@interface UIAlertView (JXTAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIAlertView (JXTAlertTools)
...
//类别，扩充属性
@interface UIActionSheet (JXTAlertTools)
@property (nonatomic, copy) CallBackBlock clickBlock;
@end

//runtime内联机制，实现set/get方法
@implementation UIActionSheet (JXTAlertTools)
- (void)setClickBlock:(CallBackBlock)block
{
    objc_setAssociatedObject(self, @selector(clickBlock), block, OBJC_ASSOCIATION_COPY);
}
...
#pragma mark - 接口
@interface JXTAlertTools () <UIAlertViewDelegate, UIActionSheetDelegate>
          ...
          otherButtonTitles:(NSString *)otherButtonTitles, ...
{
    if (iOS_Version >= 8.0)
    {
        ...
        if (popover) {
            ...
            popover.sourceRect = CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT/3);
    ...
    if (iOS_Version >= 8.0)
    {
        ...
        if (otherBtnTitleArray && otherBtnTitleArray.count)
        {
            ...
            for (int i = 0; i < otherBtnTitleArray.count; i ++) {
                ...
                UIAlertActionStyle actionStyle = (UIAlertActionStyle)styleNum.integerValue;
